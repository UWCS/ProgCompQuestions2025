...
void preload(){
    ...
    inv[MAXN] = binpow(fac[MAXN].x,MOD2-2,MOD2);
...
std::vector<Mint> convolve(const std::vector<Mint>& a, const std::vector<Mint>& b) { // for this problem, O(n^2) convolve serves just fine cuz polys are small
    // If either polynomial is empty, the product is empty
    if (a.empty() || b.empty()) {
        return {};
    }

    // Initialize result vector with appropriate size filled with zeros
    std::vector<Mint> result(a.size() + b.size() - 1, 0);
...
void solve(int &tc) {
    ...
    auto multPoly = [&](priority_queue<vector<Mint>, vector<vector<Mint>>, Compare>& polys){
        while (polys.size() > 1){
            vector<Mint> poly1 = std::move(const_cast<vector<Mint>&>(polys.top()));
            polys.pop();
            vector<Mint> poly2 = std::move(const_cast<vector<Mint>&>(polys.top()));
            polys.pop();

            vector<Mint> me = convolve(poly1, poly2);
            polys.push(me);
        }
    ...
    auto f = [&](const vector<pll> &p,const vll& s, ll C){
        int m = p.size();
        if (m == 0){
            return vector<Mint>{Mint(1)};
        }
        vector<pll> b(m);

        int n = s.size();
        int j = n-1;
        b[m-1] = {C - p[m-1].first,p[m-1].second};
        int TOT = s[j] >= b[m-1].first;

        // vector<vector<Mint>> polys(m);
        priority_queue<vector<Mint>, vector<vector<Mint>>, Compare> polys;


        for (int i = m-1; i >= 0; --i){
            b[i] = {C - p[i].first,p[i].second};
            while (j-1 >= 0 && b[i].first <= s[j-1]) {
                j--;
                TOT++;
            }
            ...
            vector<Mint> curPoly(1 + min(b[i].second,c));
            for (int t = 0; t <= min(b[i].second,c); ++t){
                curPoly[t] = choose(k,t) * perm(c,t) * perm (d,k-t);
            }
            TOT -= k;
            // polys[i] = curPoly;
            polys.push(curPoly);
