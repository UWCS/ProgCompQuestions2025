...
bool dfs(int v, int p, vector<vector<pi>>& adj, ll curWeight, vll& dist, vector<bool>& a, ll& s){
    bool contained = a[v];
    if (a[v])
        dist[v] = curWeight;
    for (auto &[c,w]: adj[v]){
        ...
        auto b = dfs(c,v,adj,curWeight+w,dist,a, s);
        if (b)
            s += 2ll*w;
        contained |= b;
    }
...
void print_memory_usage() {
    ...
    std::cout << "Memory usage: " << r_usage.ru_maxrss/1024 << " KB" << std::endl;
}
...
void solve(int& tc) {
   ...
   // print_memory_usage();
    ...
    vll dist(n+1,0);
    ll s = 0;
    dfs(1,0,adj,0,dist,a,s);


    s -= *max_element(dist.begin(),dist.end());
    print(s);
    //cout << s << " " << n << "\n";
}
